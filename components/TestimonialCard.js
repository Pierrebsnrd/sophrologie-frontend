...
export default function TestimonialCard({ message, author, date }) {
  const displayDate = (() => {
    if (!date) return null;
  ...
  return (
    <div className={styles.temoignageCard}>
      <p className={styles.temoignageText}>"{message}"</p>
      <div className={styles.temoignageFooter}>
        {displayDate && (
          <p className={styles.temoignageDate}>Publié le {displayDate}</p>
        )}
